...
export async function GET(req) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      client.send({ type: 'CONNECTED', message: 'Connected to fall detection monitor' });
      ...
      const keepAliveInterval = setInterval(() => {
        try {
          ...
          console.log(`Client ${clientId} disconnected during keepalive`);
...
export async function POST(req) {
  try {
    ...
    console.log(`Fall detected for user ${userId} at ${address}`);

    // Import and call your Python function
    // Note: You'll need to set up a Python bridge or API call here
    // For now, we'll create a mock alert message
    const alertMessage = await generateAlertMessage(userId, address);
    ...
    console.log(`Broadcasting to ${clients.length} clients:`, eventData);
...
// This should call your Python get_alerts.py function
async function generateAlertMessage(userId, address) {
  // TODO: Call your Python function here
  // For now, return a mock message
  return `Emergency: Person has fallen and is unresponsive at ${address}. ${
